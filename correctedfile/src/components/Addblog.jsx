...
//     axiosInstance.put('http://localhost:3000/blogg/edit/'+location.state.val._id,form).then((res)=>{
//       alert('Update Successful!!')
//     }).catch((err)=>{
//       console.log(err)
//     })
//   }else{
//     axiosInstance.post('http://localhost:3000/blogg/add',form).then((res)=>{
...
const Addblog = () => {
  ...
  const handleSubmit = () => {
    const request = location.state?.val
      ? axiosInstance.put(`/blog/edit/${location.state.val._id}`, form)
      : axiosInstance.post('/blog/add', form);

    request
      .then(() => {
        alert(location.state?.val ? 'Update Successful!!' : 'Blog added Successfully!!');
        navigate('/blog');
      })
      .catch((err) => {
        console.error(err);
  ...
  return (
    <Grid container justifyContent="center" style={{ marginTop: '5%' }}>
      <Grid item xs={12} sm={10} md={6} lg={5}>
        <Paper elevation={3} style={{ padding: '30px', borderRadius: '15px' }}>
          <Typography variant="h5" gutterBottom align="center">
            {location.state?.val ? 'Edit Blog' : 'Add New Blog'}
          </Typography>
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <TextField
            ...
            <Grid item xs={12}>
              <Button
                ...
                size="large"
              >
                {location.state?.val ? 'Update Blog' : 'Add Blog'}
              </Button>
            </Grid>
          </Grid>
        </Paper>
